...
from native_mounts import reconcile_mounts

cfg = read_app_config() or {}
entries = cfg.get("native_external_mounts", []) or []

try:
    result = reconcile_mounts(cfg, entries) or {}
    failed = int(result.get("failed", 0))
    print(f"Native mount reconcile complete (failed={failed})")
except Exception as exc:
    print(f"WARNING: Native mount reconcile failed: {exc}")
PY
