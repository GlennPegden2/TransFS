...
### Added
- **DB-first layout-agnostic mapping implementation plan**: Added [docs/DB_FIRST_LAYOUT_IMPLEMENTATION_PLAN.md](docs/DB_FIRST_LAYOUT_IMPLEMENTATION_PLAN.md) as the canonical feature-plan document for transitioning query maps to a database-first, layout-agnostic model in pre-alpha. The plan captures architecture context, phased implementation (runtime read path, sync assignment, category/shared-bios hardening, performance tuning, cleanup), risks, test strategy, acceptance criteria, and session-continuity guidance for future work.
- **Focused DB-first query-path helper tests**: Added `app/tests/test_db_first_query_paths.py` to validate relative path extraction for query maps with `source_dir` configured and omitted.
- **Additional DB-first helper regression coverage**: Extended `app/tests/test_db_first_query_paths.py` with an archive-marker path extraction contract test for source_dir-omitted flows.
- **Sync preserve-structure helper coverage for optional source_dir**: Extended `app/tests/test_db_first_query_paths.py` with `derive_query_map_relative_dir(...)` tests to validate source_dir-anchored and source_dir-omitted relative-directory derivation.

### Changed
- **Browse Native now marks symlinks with a tooltip icon**: Added a non-clickable symlink indicator next to symlink entries in the Native browser list (`app/webui/templates/index_complete.html`). Hovering the icon now shows the symlink target path sourced from `/api/browse` metadata (`app/api.py`).
- **Browse Native mount awareness expanded to mount ancestry**: Native browser entries now also show the same link icon when a folder contains a configured managed mount deeper in its subtree (for example `Systems/3DO`). Tooltips now show either direct mount source or descendant mount source based on `/api/browse` metadata.
- **RetroNAS startup now auto-bridges Native into configured filestore when paths differ**: `start_transfs_retronas.sh` now creates `filestore/Native -> {retronas_path}/Native` if the target exists and no conflicting path is present. This restores expected Browse Native visibility in profiles where app config points at `/mnt/filestorefs` while RetroNAS data remains under `/data/retronas`.
- **Web API startup also enforces RetroNAS Native bridge in bind-mounted runtime profiles**: `app/main.py` now creates `filestore/Native -> {retronas_path}/Native` on FastAPI startup when running inside RetroNAS and the link is missing. This keeps the bridge behavior active even when launcher scripts are stale or not rebuilt.
- **External mounts now populate a dedicated non-FUSE share root (`/mnt/filestore`)**: Native External Mounts were refactored so configured entries mount under `/mnt/filestore` instead of being treated as direct `Native/` filestore overlays. This preserves a single direct-access share for clients/tools while keeping `/mnt/transfs` as the virtual layer.
- **Native External Mounts expanded beyond SMB/CIFS**: Added mount-type support (`cifs`, `nfs`, `bind`) across API, runtime mount reconciliation, config linting, and web UI. Existing SMB entries remain backward-compatible with default `mount_type: cifs` behavior.
- **SMB `TransFSNative` share now points at `/mnt/filestore`**: Samba template and startup now expose the new aggregate external mount root for direct access workflows.
- **Initial DB-first query-map runtime slice (Phase 1)**: Updated `app/vfs/dirlisting.py` and `app/transfs.py` so query-map database queries no longer assume implicit `source_dir: Software`; `source_dir` is now optional and only applied as a narrowing filter when explicitly configured.
- **DB-first query-map runtime matching hardened (Phase 1, slice 2)**: Refactored DB-only `getattr`/`open` query-map resolution in `app/transfs.py` to resolve targets from DB candidate sets and cached map-root query results rather than reconstructing source paths from config assumptions. Added helper functions for virtual-name and relative-dir derivation from DB records to support source_dir-optional behavior.
- **Query-map preserve-structure path extraction now supports source_dir omission**: `_extract_relative_path` in `app/vfs/dirlisting.py` now falls back to `/Native/{local_base_path}/` anchoring when no `source_dir` is configured.
- **Phase 2 sync assignment now treats source_dir as optional query-map filter**: Updated `app/sync_database.py` so query-map scan assignment no longer defaults `source_dir` to `Software`; source_dir narrowing is applied only when explicitly configured. Preserve-structure relative-dir derivation now also supports source_dir-omitted query maps via `vfs.pathutils.derive_query_map_relative_dir(...)`.
- **App module structure refactored into logical packages**: Flat Python modules in `app/` reorganised into five new sub-packages to improve navigability and separation of concerns. All imports updated throughout; no functional changes.
  - `app/data_provider/` — data provider abstraction layer (`__init__.py`, `cache.py`, `db.py`, `hybrid.py`, `init.py`)
  - `app/retronas/` — RetroNAS compatibility layer (was `retronas_support.py`; `projection.py` remains as a re-export shim)
  - `app/fuse/` — FUSE support modules (`passthrough.py`, `adapter.py`, `cache_warmer.py`, `prewarm.py`; `transfs.py` stays flat as the FUSE entry point)
  - `app/archive/` — ZIP/7z archive access (`zippath.py`, `utils.py`)
  - `app/vfs/` — virtual filesystem resolution layer (`pathutils.py`, `dirlisting.py`, `sourcepath.py`, `filetypes.py`)
  - `smb_config.py` and `native_mounts.py` remain flat (shell-script dependencies and storage management respectively)
- **Web UI assets moved under `app/webui/`**: `app/static/` → `app/webui/static/` and `app/templates/` → `app/webui/templates/`. The HTTP mount path `/static` is unchanged so no browser-side URLs are affected. Updated `app/main.py` directory references.

### Added
- **28 new MiSTer system stubs in `app/config/clients/default/mister.yaml`**: Added stub entries for systems present in the Game Populator database but not yet covered in the default MiSTer client config. Systems added: AVision (Entex), Astrocade (Bally), BBCBridgeCompanion, CD-i (Philips), Casio PV-1000, ChannelF (Fairchild), CreatiVision (VTech), Gamate (Bit Corp), Game & Watch (Nintendo), MegaCD (Sega), MyVision (Nichibutsu), N64 (Nintendo), NeoGeo-CD (SNK), NeoGeoPocket (SNK), Odyssey2 (Magnavox), PSX (Sony), PocketChallengeV2 (Benesse), PokemonMini (Nintendo), S32x (Sega), SCV (Epoch), SGB (Nintendo), Saturn (Sega), SuperVision (Watara), SuperVision8000 (Bandai), TGFX16-CD (NEC), VC4000 (Interton), WonderSwan (Bandai), WonderSwanColor (Bandai). Optical systems (PSX, Saturn, MegaCD, TGFX16-CD, NeoGeo-CD, CD-i) use `CDs:` maps with CHD/CUE/BIN extensions; cartridge systems use `ROMs:` maps. All existing entries are unchanged (non-destructive merge). Extension data sourced from [Game Populator](https://github.com/cosmickatamari/game-populator); manufacturer/display names cross-referenced from [RetroNAS](https://github.com/retronas/retronas).

- **`tools/generate_mister_stubs.py`**: New development tool that cross-references Game Populator and RetroNAS system databases against the current `default/mister.yaml` and generates YAML stubs for uncovered systems. Supports `--write` (append to mister.yaml) and `--sources` (create stub source yamls). Non-destructive: existing systems are never modified.

### Fixed
- **RetroNAS testbed image build no longer fails on `/bin/sh` pipefail option**: Updated `Dockerfile.retronas-testbed` to use `set -eu` in the main build `RUN` layer so builds succeed on shells that do not support `set -o pipefail`.
- **RetroNAS testbed image now installs `python3-venv`**: Updated `Dockerfile.retronas-testbed` package install list so runtime venv creation succeeds during image build even when the base image lacks ensurepip/venv support by default.
- **RetroNAS testbed build now uses correct script/tool paths**: Updated `Dockerfile.retronas-testbed` to run `render_platform_scripts.py` from `app/build_scripts` and copy `tools/` into `/opt/transfs/tools` so generated platform scripts and client-import tooling are available during image build.
- **RetroNAS testbed now launches TransFS runner by default**: Updated `docker-compose.yml` (`retronas-testbed-runtime`) to run `/opt/transfs/platform/retronas/start_transfs_retronas.sh` so FUSE, web API, native mount reconcile, and Native bridge logic execute automatically on `compose up`.
- **RetroNAS startup runner now honors generated runtime env defaults**: Updated `start_transfs_retronas.sh.j2` to source `/etc/default/transfs-retronas` at startup so `TRANSFS_PYTHON_BIN` points at the prebuilt venv and dependency checks/install paths do not fall back to system Python.
- **Browse Native now marks failed configured mounts under the RetroNAS Native bridge**: Updated `/api/browse` to treat `Native/...` browse paths as aliases of configured native mount targets, emit `mount_state`/`mount_error`, and keep descendant mount metadata bridge-aware even when the live mount is missing.
- **Browse Native now shows a warning icon for failed configured mounts**: Updated the Web UI native browser row rendering to show a non-clickable warning icon with a tooltip describing the mount failure and configured source, so entries like `3DO` still surface actionable diagnostics when auth is missing.
- **Foundation test runs in RetroNAS testbed no longer appear stuck due to invalid pytest working directory**: Updated `/api/run-tests` in `app/api.py` to resolve pytest working directory dynamically (`/app`, `/opt/transfs`, then current directory) instead of hardcoding `/app`, preventing silent launch failures like `[Errno 2] No such file or directory: '/app'` in plugin-style runtimes.
- **RetroNAS testbed now runs live workspace API/tests by default**: Added bind mounts for `./app -> /opt/transfs/app` and `./tests -> /opt/transfs/tests` in `docker-compose.yml` so the testbed container does not drift to stale in-image API code after rebuilds, keeping Web UI test behavior aligned with the current repository state.
- **RetroNAS testbed now mounts root requirements into `/opt/transfs`**: Added `./requirements.txt -> /opt/transfs/requirements.txt` in `docker-compose.yml` for the `retronas-testbed-runtime` service so installer-driven venv setup includes runtime dependencies (for example `trio` and `pyfuse3`) when app code is bind-mounted.
- **`/api/run-tests` now reports explicit startup errors**: When pytest launch fails before task creation, the endpoint now returns `{"status": "error", ...}` so the UI can stop cleanly instead of appearing to run indefinitely on malformed/partial responses.
- **`/api/test-results/last` robust against route-order shadowing**: Added a guard in `get_test_results(task_id)` to delegate `task_id == "last"` to `get_last_test_results()`, preventing false `Task last not found` responses when dynamic route matching takes precedence.
- **RetroNAS startup now self-heals missing `pycryptodome`**: `start_transfs_retronas.sh` now checks `import Crypto` and installs `pycryptodome` into the configured runtime venv if missing, preventing Web UI/API startup failures after environment drift or partial installs.
- **RetroNAS installer now validates Crypto import**: `3rd-party/retronas/ansible/install_transfs.yml` now explicitly verifies `from Crypto.Cipher import AES` after dependency installation so setup fails fast if the runtime venv is incomplete.
- **RetroNAS installer now fails fast when `requirements.txt` is missing**: `3rd-party/retronas/ansible/install_transfs.yml` now checks for `/opt/transfs/requirements.txt` before building filtered requirements, preventing silent partial installs in container layouts where only subfolders are mounted.
- **Atari 800 default source YAML no longer breaks startup**: Fixed malformed YAML in `app/config/sources/default/Atari/Atari800.yaml` by commenting stray plain text (`System Roms`) so config loading no longer throws `yaml.scanner.ScannerError` during runtime initialization.
- **Native external mounts now target `/mnt/filestorefs` (not `/mnt/filestore`)**: Updated `app/native_mounts.py` so managed external mount targets are created under `/mnt/filestorefs`, matching foundation test expectations and container volume layout. Target normalization remains backward-compatible with legacy `/mnt/filestore/...` paths.
- **Removed non-deterministic top-level client assertion from foundation tests**: Deleted `TestClientMappings::test_only_expected_clients_at_toplevel` from both `tests/test_foundation.py` and `app/tests/test_foundation.py` because `/mnt/transfs` top-level contents are no longer deterministic across runtime profiles/config sets.
- **Removed system-specific Archimedes write test from foundation suite**: Deleted `TestWriteCapabilities::test_can_write_and_cleanup_mister_archimedes_folder` from the root foundation test file so the generic suite no longer depends on a system-specific MiSTer layout.
- **Native browsing and RetroNAS filestore defaults now use `/mnt/filestorefs`**: Updated the web UI bootstrap, checked-in `app.yaml`, API fallbacks, and RetroNAS installer template so the native browser root defaults to `/mnt/filestorefs/Native` instead of `/data/retronas/Native`. This keeps RetroNAS content aligned with the shared filestore root used by other data sources.
- **Browse Native now opens at the filestore root**: Changed the UI template context so the native browser starts at `/mnt/filestorefs` rather than `/mnt/filestorefs/Native`, while the browser still navigates into the `Native/` subtree for content.
- **Legacy `/data/retronas` filestore fallbacks removed from runtime mount/projection paths**: Updated `app/native_mounts.py`, `app/retronas/__init__.py`, and `app/transfs.py` so default/fallback filestore resolution now uses `/mnt/filestorefs`. This keeps RetroNAS content roots and configured native mount runtime paths aligned when config fallback logic is exercised.
- **Database feature-flag fallback path now follows configured filestore**: Updated `app/feature_flags.py` so `database.path` defaults to `{filestore}/.transfs_metadata.db` (currently `/mnt/filestorefs/.transfs_metadata.db`) instead of hardcoded `/data/retronas/.transfs_metadata.db`.
- **External CIFS mounts now normalize Windows-style usernames for kernel mounts**: `app/native_mounts.py` now converts `.\\user` to `user` and parses `DOMAIN\\user` into `username=user` plus `domain=DOMAIN` for `mount.cifs` options. This prevents false auth failures where `smbclient` succeeds but kernel CIFS mount returns `mount error(13): Permission denied`.
- **RetroNAS testbed image/runtime now supports CIFS/NFS mount helpers**: `Dockerfile.retronas-testbed` now installs `cifs-utils`, `smbclient`, and `nfs-common`; `docker-compose.yml` adds testbed runtime capability/security settings required by `mount.cifs` helper behavior.
- **Foundation/system test runs no longer get stuck "In progress" due to pytest stdout backpressure**: Updated `/api/run-tests` in `app/api.py` to continuously drain subprocess output on a background thread instead of reading a single line per poll cycle. This prevents pytest from blocking on a full stdout pipe and allows status transitions to complete reliably.

- **Test runner now resolves pytest file paths across standalone and RetroNAS layouts**: Updated `/api/run-tests` in `app/api.py` to resolve test files under `/tests`, `/opt/transfs/tests`, or `/app/tests` (including wildcard runs), preventing immediate failures like `file or directory not found: /tests/test_foundation.py` in plugin environments.

- **Foundation/UI test detail parsing fixed for RetroNAS-style pytest nodeids**: Updated `get_test_results` parsing in `app/api.py` to recognize lines like `../../tests/test_foundation.py::... <- ../opt/transfs/tests/... PASSED/FAILED`, so `results.tests` is populated with per-test status/output and failure detail panes in the UI are no longer blank.

- **Test Results UI now exits gracefully on stale/unknown task states**: Updated polling logic in `app/webui/templates/index_complete.html` so `/api/test-results/{task_id}` responses such as `not_found` or `error` clear the in-progress indicator with a clear message instead of leaving the UI appearing to run forever.

- **TransFS startup no longer crashes on invalid native mount credentials**: Hardened `reconcile_mounts` in `app/native_mounts.py` to catch per-entry mount exceptions and report failed entries instead of aborting startup when an enabled mount has `guest: false` but missing/invalid password credentials.

- **FastAPI startup no longer hard-fails on incompatible MEGA dependency**: Updated `app/api.py` to make `mega` import optional at runtime. If the installed MEGA client package is incompatible with Python 3.12, API startup continues and MEGA download paths emit a clear "MEGA downloader unavailable" message instead of crashing the service.

- **RetroNAS client-config import now regenerates all CIFS client YAMLs**: The RetroNAS import API/UI path had remained wired to the legacy MiSTer-only behavior even after `tools/import_mister_cifs.py` was generalized to support every `install_*_cifs.yml` playbook. The importer endpoint now runs in `--all` mode and regenerates `config/clients/retronas/*.yaml` for every discovered RetroNAS CIFS client instead of only `mister.yaml`.

- **RetroNAS Samba include now lands at the end of `smb.conf`**: The installer had been inserting `include = /etc/samba/transfs.conf` immediately after `[global]` in `transfs_managed` mode and as an `include` option inside a `TransFS` section in `retronas_managed` mode. Both placements can make Samba parse global options as share options. The installer now removes any misplaced include and re-adds the TransFS include at EOF so the injected share definitions stay separate from Samba's global settings.

- **`configure_retronas.sh.j2` now sets `active_client_config: retronas`**: The Ansible installer's configure script now explicitly switches the active client config set to `retronas` in `app.yaml`. Previously it was left as `default`, so the `mister.yaml` generated by `import_mister_cifs.py` (written to `clients/retronas/`) was never loaded and the FUSE virtual filesystem used the old `Canonical/` path layout instead of the correct `{filestore}/Native/{category}/{src}` layout.
...
### Added
- **Ansible playbook — non-destructive Samba share injection for `transfs_managed` mode**: Replaced the previous approach of overwriting `/etc/samba/smb.conf` with a safe include pattern that mirrors `retronas_managed`. A new template `transfs-managed-shares.conf.j2` deploys both `[TransFS]` and `[TransFSNative]` share definitions to `/etc/samba/transfs.conf`, and a `lineinfile` task injects `include = /etc/samba/transfs.conf` into the `[global]` section of the existing smb.conf — leaving all other RetroNAS (or system) configuration intact. Both shares verified with `testparm` and `smbclient` inside the retronas:testbed container.
...
- **Ansible playbook — postgres reachability check no longer requires `ping`**: Replaced `ping -c 1 postgres` (not installed in the RetroNAS container) with a Python3 TCP socket check on port 5432, avoiding a false-positive warning on every run.
